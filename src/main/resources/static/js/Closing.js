layui.use(['table','layer','jquery'], function(){
    ...
    table.render({
        elem: '#demo'
        ,url: 'http://api.insurance.com/api/item/Comprehensive_adjustmentVerify/list2' //数据接口
        ,page: true //开启分页
        ,limit:5 //每一页显示5条数据
    ...
    table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'adopt'){
            layer.msg('结案完成');
            $.ajax({
                url: "http://api.insurance.com/api/item/Comprehensive_adjustmentVerify/closing?verify_id="+verify_id,
                async: true,
                dataType: "json",
